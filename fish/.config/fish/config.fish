...
# set emacs to open maximized by default, if not in term only environment (ssh)
if ! string match -qr 'SSH.*' (printenv)
    abbr emacs "emacs -mm"
    ...
    #Set default editor
    set -gx EDITOR emacs
    set -gx SUDO_EDITOR emacs
end

# Set this to ensure that emacs within terminal doesn't look weird
